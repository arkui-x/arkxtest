...
class UserConfigManager(object):
    ...
    def get_aosp_device(self, target_name):
        for node in self.config_content.findall(target_name):
            data_dic = {}
            if node.get("label", None) is None:
                continue
            if node.attrib["type"] != "usb-hdc" and \
                    node.attrib["type"] != "usb-adb" and \
                    node.attrib["label"] != "aosp":
                continue
            data_dic["usb_type"] = node.attrib["type"]
            for sub in node:
                if sub.text is None:
    ...
    def get_ios_device(self, target_name):
        for node in self.config_content.findall(target_name):
            data_dic = {}
            if node.get("label", None) is None:
                continue
            if node.attrib["type"] != "usb-ios" and node.attrib["label"] != "ios":
                continue
            data_dic["usb_type"] = node.attrib["type"]
            for sub in node:
                if sub.text is None:
