...
from aosp.environment.dmlib import DeviceConnector
from aosp.constants import UsbConst

__all__ = ["ManagerAospDevice"]

LOG = platform_logger("ManagerAospDevice")


@Plugin(type=Plugin.MANAGER, id=ManagerType.aosp_device)
class ManagerAospDevice(IDeviceManager, IFilter):
    """
    ...
    def __init__(self):
        ...
        self.support_types = ["device_aosp", "device"]
        self.wait_times = 0
    ...
    def _start_device_monitor(self, environment="", user_config_file=""):
        ...
        device = UserConfigManager(
            config_file=user_config_file, env=environment).get_aosp_device(
            "environment/device")
        if device:
            try:
        ...
        try:
            ...
            for device in self.devices_list:
                if device_option.matches(device):
                    ...
                    LOG.debug("Allocate device sn: {}, type: {}".format(
    ...
    def release_device(self, device):
        ...
        try:
            ...
            LOG.debug("Free device sn: {}, type: {}".format(
    ...
    def lock_device(self, device):
        ...
        try:
            ...
            LOG.debug("Lock device sn: {}, type: {}".format(device.__get_serial__(), device.__class__.__name__))
        finally:
    ...
    def find_or_create(self, idevice):
        ...
        try:
            ...
            if device is None:
                ...
                if self.device_connector.usb_type == DeviceConnectorType.hdc and shutil.which("hdc"):
                    device_instance.usb_type = DeviceConnectorType.hdc
                else:
                    device_instance.usb_type = "usb-adb"
                LOG.debug("Create device({}) host is {}, "
                          "port is {}, device sn is {}, usb type is {}".format
...
class ManagedDeviceListener(object):
    ...
    def device_changed(self, idevice):
        ...
        LOG.debug("Device changed to {}: {} {} {} {}".format(
    ...
    def device_connected(self, idevice):
        ...
        LOG.debug("Set device {} {} to true".format(
    ...
    def device_disconnected(self, disconnected_device):
        ...
        LOG.debug("Device disconnected: {} {} {} {}".format(
