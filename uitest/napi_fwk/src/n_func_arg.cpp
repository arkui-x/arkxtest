...
bool NFuncArg::InitArgs(std::function<bool()> argcChecker)
{
    ...
    if (status != napi_ok) {
        HILOG_ERROR("Cannot get num of func args for %{public}d", status);
        return false;
    }
    if (argc) {
        ...
        if (status != napi_ok) {
            HILOG_ERROR("Cannot get func args for %{public}d", status);
            return false;
        }
    }
    SetArgc(argc);
...
bool NFuncArg::InitArgs(size_t argc)
{
    return InitArgs([argc, this]() {
        ...
        if (argc != realArgc) {
            HILOG_ERROR("Num of args recved eq %zu while expecting %{public}zu", realArgc, argc);
            return false;
        }
        return true;
    });
