...
bool NVal::AddProp(vector<napi_property_descriptor> &&propVec) const
{
    if (!TypeIs(napi_valuetype::napi_object)) {
        HILOG_ERROR("INNER BUG. Prop should only be added to objects");
        return false;
    }
    napi_status status = napi_define_properties(env_, val_, propVec.size(), propVec.data());
    if (status != napi_ok) {
        HILOG_ERROR("INNER BUG. Cannot define properties because of %{public}d", status);
        return false;
    }
    return true;
}
...
bool NVal::AddProp(string propName, napi_value val) const
{
    ...
    if (status != napi_ok) {
        HILOG_ERROR("INNER BUG. Cannot set named property because of %{public}d", status);
        return false;
    }
    return true;
}
