...
#ifndef UITEST_LIBN_N_CLASS_H
#define UITEST_LIBN_N_CLASS_H
...
public:
    ...
    {
        if (!env || !objStat) {
            HILOG_ERROR("Empty input: env %d, obj %d", env == nullptr, objStat == nullptr);
            return nullptr;
        }
        T *t = nullptr;
        napi_status status = napi_unwrap(env, objStat, (void **)&t);
        if (status != napi_ok) {
            HILOG_ERROR("Cannot umwarp for pointer: %d", status);
            return nullptr;
        }
        return t;
    }
